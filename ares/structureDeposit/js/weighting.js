        ...
        methods: {
                ...
                $http(_this.getDetailUrl,true,params, true)
                    .then(res => {
                        if(res.retcode == 'success'){
                            ...
                            if(_this.infos.weightedRate && _this.infos.weightedRate != ''){
                                // _this.infos.weightedRate = (_this.infos.weightedRate*100).toFixed(4);
                                _this.infos.weightedRate = _this.toFixeda((_this.infos.weightedRate));
                            };
                            //处理已审批
                            _this.approvedInfo(res.data.approveInfoList);
                            //处理利率日期
                            _this.getLilvDate();
            ...
            approvedInfo(arr){
                for(var i=0;i<arr.length;i++){
                    if(arr[i].approveState == this.role){
                        this.approvedInfos = arr[i];
                        break;
                    }
                };
            },
            ...
            tosubInfo(i){
                ...
                if(i == '1' && this.approvedInfos.rebackFlag == 1 && this.infos.weightedRate == ''){
                    vant.Toast('请输入加权平均利率');
                    return;
                };
                ...
                if(i == '1' && this.approvedInfos.rebackFlag == 1){
                    params.weightedRate = _this.infos.weightedRate.toString();
                }
