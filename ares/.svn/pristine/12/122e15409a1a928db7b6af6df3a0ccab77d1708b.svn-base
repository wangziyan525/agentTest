function action() {
    new Vue({
        ...
        data: {
            ...
            getListUrl: 'bail/findApproveList.xa',//列表接口
            ...
            filterTimeText:'近一周',
            filterStatusText:'全部审批状态',

            showPickerStatus:false, //状态遮罩层
            chooseList:[
                {name:'全部审批状态',isChoose:true},
                {name:'审批通过',isChoose:false},
                {name:'审批拒绝',isChoose:false},
            ],
        ...
        methods: {
            ...
            getList(){
                let _this = this;
                let params = {};
                params.approveStatus = _this.active;
                ...
                if(_this.active == 0){
                    ...
                    params.startTime = _this.startTime;
                    params.endTime = _this.endTime;
                    if(_this.filterStatusText == '全部审批状态'){
                        params.approveFlag = 2;
                    }else if(_this.filterStatusText == '审批通过'){
                        params.approveFlag = 0;
                    }else if(_this.filterStatusText == '审批拒绝'){
                        params.approveFlag = 1;
                    }
                };
                $http(_this.getListUrl,true,params, true,60)
                    .then(res => {
                        if(res.retcode == 'success'){
                            if(res.data.voList){
                                var arr = res.data.voList;
                                if(arr.length == 0){
                                    if(_this.pagenum == 1){
            ...
            //--------------------------------------------------------------------- 跳转
            toDetail(no){
                window.location.href = './invoiceDetail.html?batchNo=' + no;
            },
            ...
            selectStatus(i){
                for(var j=0;j<this.chooseList.length;j++){
                    this.$set(this.chooseList[j],'isChoose',false);
                };
                this.$set(this.chooseList[i],'isChoose',true);
            },
            ...
            selectStatused(){
                var str = '';
                for(var j=0;j<this.chooseList.length;j++){
                    if(this.chooseList[j].isChoose){
                        str = this.chooseList[j].name;
                        break;
                    }
                };
