        ...
        methods: {
                    ...
                    .then(res => {
                        if(res.retcode == 'success'){
                            if(res.data=='headoffice'){
                                that.jumpFlag = '1'
                                if(that.detail.currentStage == '5'){
                                    that.nextUserFlag = true
                                    that.finishShow = '1'
                                    that.getNextApproveUser();
                                }else if(that.detail.currentStage == '6'){
                                    that.nextUserFlag = false
                                    that.finishShow = '0'
                                }else{
                                    that.nextUserFlag = true
                                    that.finishShow = '0'
                                ...
                                that.jumpFlag = '0'
                                if(that.detail.currentStage=='1' || that.detail.currentStage=='2'){
                                    that.nextUserFlag = true
                                    that.getNextApproveUser();
                                }else{
                                    that.nextUserFlag = false
                        ...
                        if (res.retcode == 'success'){
                            ...
                            for(var i=0;i<that.detail.approveRecordList.length;i++){
                                if(that.detail.approveRecordList[i].stage=='0' || that.detail.approveRecordList[i].stage=='1' || that.detail.approveRecordList[i].stage=='2' || that.detail.approveRecordList[i].stage=='3' || that.detail.approveRecordList[i].stage=='4' || that.detail.approveRecordList[i].stage=='5' || that.detail.approveRecordList[i].stage=='6'){
                                    if(that.detail.approveRecordList[i].status!='0'){
            ...
            submitBindtap(num) {
                ...
                if(that.status=='2'){
                    ...
                    if(that.finishFlag=='0'){
                        if(that.nextUserFlag == true){
                ...
                if(that.finishShow=='1'){
                    params.finishFlag = that.finishFlag
                }
                    ...
                    $http(baseUrl.submitApproveInfo,true,params, true)
                        .then(res => {
                            if(res.retcode == 'success'){
                                $.alert("","提交成功",function () {
                                    if(that.jumpFlag == '1'){
                                        window.location.href = 'registerApproveListZH.html';
                                    }else{
