...
$(document).ready(function(){
	...
	$("#typeList").on("click",".gridsLine",function(){
		...
		window.location.href = './datalist.html?secondVideoTypelisy='+appVideo.secondVideoTypelisy+"&id="+appVideo.id+"&keyWord="+appVideo.keyWord+"&recommend="+appVideo.recommend+"&code="+wxcode;
	});
			...
			if(data.retcode=="0"){
				...
				if(list.length>0){
					for (var i=0;i<list.length;i++){
						var temp = {};
						temp=list[i];
						temp.secondVideoType = temp.secondVideoType ? temp.secondVideoType : '';
...
function search1(){
	...
	appVideo.id="";
	appVideo.recommend="";
	window.location.href = './datalist.html?keyWord='+appVideo.keyWord;
	// $("#videoPage").show();
	// $("#indexPage").hide();
	// queryVideoList();
	// $("#searchInputs").blur();
...
//播放
// function play(id) {
// 	window.location.href = './datalist.html?voideId='+id;
// }
