var baseUrl = {
    queryList: "retailsalescheck/getProblemApproveList.xa",
    getAssignProblemPower: 'retailsalescheck/getAssignProblemPower.xa'
}
var vm
function initFun () {
    vm = new Vue({
        ...
        methods:{
            ...
            getAssignProblemPower() {
                var that = this;
                var token = localStorage.getItem("X-Token");
                let param = {

                }
                $http(baseUrl.getAssignProblemPower,true,param, true)
                    .then(res => {
                        if(res.retcode == 'success'){
                            that.assignPowerFlag = res.data
                            that.getList()
            ...
            getList() {
                ...
                var token = localStorage.getItem("X-Token");
                let param = {
                    submitDate:that.receiveDate,
                    pageNum: that.pageNum,
                    pageSize: that.pageSize,
                    approveStatus:that.tab,
                }
            ...
            jump(item){
                if(this.tab=='0'){
                    window.location.href = 'rectifyApprove.html?id='+item.id+'&tab='+this.tab
                }else{
                    window.location.href = 'rectifyDetail.html?id='+item.id+'&tab='+this.tab
                }
