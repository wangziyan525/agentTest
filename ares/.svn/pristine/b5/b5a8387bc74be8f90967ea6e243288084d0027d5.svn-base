function action() {
    new Vue({
        ...
        data: {
            ...
            getListUrl: 'certExpire/getCertExpireApprovalList.xa',//列表接口
        ...
        mounted: function () {  
            ...
            //获取列表（未审批）
            if(sessionStorage.getItem("expMaintenanceTab") && sessionStorage.getItem("expMaintenanceTab") == 1){
                this.getTimeDuanStartTimes1();
                this.list = [];
                this.active = 1;
                this.filterTimeText = sessionStorage.getItem("expMaintenanceTimeText");
                this.filterStatusText = sessionStorage.getItem("expMaintenanceStatusText");
                this.startTime = sessionStorage.getItem("expMaintenanceStartTime");
                this.endTime = sessionStorage.getItem("expMaintenanceEndTime");
                this.getList();
            }else{
                //获取时间段
        ...
        methods: {
            ...
            getList(){
                let _this = this;
                let params = {};
                params.approval = _this.active + '';
                if(_this.active == 1){
                    params.companyName = '';
                    params.date = _this.startTime + '/' + _this.endTime;
                    ...
                    }else if(_this.filterStatusText == '已转办'){
                        params.approvalStatus = '2';
                    }
                }else{
                    params.approvalStatus = '';
                    params.date = '';
                    params.companyName = _this.searchName;
                }
                    ...
                    .then(res => {
                        if(res.retcode == 'success'){
                            if(res.data.role == '1'){
                                ...
                                _this.chooseList = ['全部','审批通过','审批拒绝','已转办'];
                            }else{
                                _this.isController = false;
                                _this.chooseList = ['全部','审批通过','审批拒绝'];
                            };
                            if(res.data.certificateExpireRecordPageInfo.list){
                                var arr = res.data.certificateExpireRecordPageInfo.list;
                                if(arr.length == 0){
                                    if(_this.pagenum == 1){
            ...
            //--------------------------------------------------------------------- 跳转
            toDetail(tranSeq){
                window.location.href = './detail.html?tranSeq=' + tranSeq;
                if(this.active == 1){
                    this.getSession();
                }
            },
            ...
            getSession(){
                sessionStorage.setItem("expMaintenanceTab",this.active);
                sessionStorage.setItem("expMaintenanceTimeText",this.filterTimeText);
                sessionStorage.setItem("expMaintenanceStatusText",this.filterStatusText);
                sessionStorage.setItem("expMaintenanceStartTime",this.startTime);
                sessionStorage.setItem("expMaintenanceEndTime",this.endTime);
            },
            ...
            isInTimeDuan(){
                if(this.endTime == this.todayTime){
                    if(this.startTime == this.todayTime){
                        this.chooseTimeDuan = '0';
                    }else if(this.startTime == this.weekTime){
                        this.chooseTimeDuan = '1';
