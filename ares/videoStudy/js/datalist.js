var appVideo = {
	...
	// shareVideo:base.context + "/learnVideo/findVideoInfoById", //分享的视频
	base: '/wxqy',
	initFlag: false,
	saveCacheFlag: true,
	keyWord: "",
...
appVideo.id = GetQueryString('id') ? GetQueryString('id') : '';
appVideo.keyWord = GetQueryString('keyWord');
appVideo.recommend = GetQueryString('recommend') ? GetQueryString('recommend') : '';
appVideo.secondVideoTypelisy = GetQueryString('secondVideoTypelisy');
queryVideoList();
$(document).ready(function () {
	...
	wx.config({
		...
		jsApiList: ['hideAllNonBaseMenuItem', 'onHistoryBack', 'chooseImage', 'uploadImage', 'previewImage', 'onMenuShareAppMessage', 'hideMenuItems', 'showMenuItems', 'hideOptionMenu'] // 必填，需要使用的JS接口列表，所有JS接口列表见附录2
	});
	wx.ready(function () {
		// wx.closeWindow();
		wx.hideAllNonBaseMenuItem();
		// wx.onHistoryBack( function (run) {
		// 	if(appVideo.fanhui){
		// 		appVideo.fanhui = !appVideo.fanhui
		// 		$('.page2').hide();
		// 		$('.videoWrappage2').html("");
		// 		$('.page1').show();
		// 	}else {
	...
	$("#searchClear").on("click", function () {
		console.log("appVideo.keyWord", appVideo.keyWord)
		appVideo.keyWord = "";
		if (appVideo.id) {
			appVideo.pageNum = 1;
			appVideo.loading = true;
			queryVideoList1();
		} else {
			$("#videoPage").hide();
			$("#indexPage").show();
	...
	$("#typeList").on("click", ".gridsLine", function () {
		...
		appVideo.secondVideoTypelisy = $(this).attr("data_secondVideoTypelisy");
		appVideo.recommend = $(this).attr("data_recommend");
		appVideo.content = $(this).find('.videoTitel').text();
		// queryVideoList();
		// $("#videoPage").show();
		// $("#indexPage").hide();
		window.location.href = './datalist.html?secondVideoTypelisy=' + appVideo.secondVideoTypelisy;
	});
...
function queryVideoList() {
	...
	if (appVideo.secondVideoTypelisy && appVideo.secondVideoTypelisy.length > 0) {
		$('#qiehuan').hide();
		$("#qiehuan1").hide();
		$("#qiehuan2").hide();
		$("#qiehuan3").hide();
		console.log(appVideo.secondVideoTypelisy, 'appVideo.secondVideoTypelisy');
		var datalist = appVideo.secondVideoTypelisy
		appVideo.secondVideoType = datalist[0]
		if (appVideo.secondVideoType == '1') {
			$("#qiehuan1").attr('class', 'active');
			$("#qiehuan1").siblings().attr('class', '');
		} else if (appVideo.secondVideoType == '2') {
			$("#qiehuan2").attr('class', 'active');
			$("#qiehuan2").siblings().attr('class', '');
		} else if (appVideo.secondVideoType == '3') {
			$("#qiehuan3").attr('class', 'active');
			$("#qiehuan3").siblings().attr('class', '');
		}
		console.log(appVideo.secondVideoType, 'appVideo.secondVideoType');
	...
	// }
	// 		}
	var H = $('.topFixed').height()+'px';
	$('.holdPlace').css('height',H)
	queryVideoList1();
	share();
}
...
function queryVideoList1() {
	...
	appVideo.loading = false;
	$.ajax({
		url: appVideo.queryList,
		type: 'POST',
		data: JSON.stringify({
			/*'userId':JSON.parse($.cookie("user")).userid,*/
			'typeId': appVideo.id,
			'title': decodeURI(appVideo.keyWord),
			'recommend': appVideo.recommend,
			'secondVideoType': appVideo.secondVideoType,
			"pageNum": appVideo.pageNum,
			'pageSize': appVideo.pageSize,
			learnstatus: appVideo.learnstatus
		}),
		'content-Type': 'application/json;charset=utf-8;',
		beforeSend: function (xhr) {
			$.showLoading();
		...
		success: function (data) {
			if (data.retcode == "0") {
				if(appVideo.pageNum==1){
					$('.videoWrap').html("");
				}
				$('.notips').hide();
				$('.zwsj').hide();
				var list = data.data.list;
				appVideo.splist = list
				if (list && list.length == 0 && appVideo.pageNum == 1) {
					appVideo.loading = false;
					if (appVideo.keyWord) {
						$('.notips').text('暂无与“' + decodeURI(appVideo.keyWord) + '”相关的视频');
					} else {
						$('.notips').text("暂无视频");
					}
					$('.notips').show();
					...
					if (appVideo.pageSize > list.length) {
						appVideo.loading = false;
					} else {
						appVideo.loading = true;
					}
					if (list.length > 0) {
						for (var i = 0; i < list.length; i++) {
							...
							if(list[i].kebaoName==undefined||list[i].kebaoName==''){
								temp.ifSeeKebao = "display:none";
							}else{
								temp.ifSeeKebao = "display:block";
								var kebaoTemp = $('#kebaoTemp').html();
								var kebaoList = list[i].kebaoName.split(',');
								console.log(kebaoList,'kebaoList')
								for(var j = 0;j<kebaoList.length;j++){
									var kbTemp = {};
									kbTemp.kebaoName = kebaoList[j];
									kbHtmls +=appVideo.fillTemplate(kebaoTemp, kbTemp);
								}	
							}
							
							htmls += appVideo.fillTemplate(videoTemp, temp);
							$('.videoWrap').append(htmls);
							var element = $('.videoWrap').find('.videoBox:last');
							element.find('.kebao').append(kbHtmls);
							$('.videoWrap').show()
						}
						initVideo();
					}
	...
	for (var p = videos.length - 1; p >= 0; p--) {
		(function () {
			...
			if (localStorage.getItem($(videos[p]).attr("id"))) {//初始化缓存时间
				...
				// 播放
				$(videos[p]).attr("myCurrentTime2", localStorage.getItem($(videos[p]).attr("id")));
				$(videos[p]).attr("myMaxTime2", localStorage.getItem($(videos[p]).attr("id")));
				var seeTime = secToTime($(videos[p]).attr("myCurrentTime2"));
				$("#myCurrentTime2" + $(videos[p]).attr("id")).text("上次观看" + seeTime);
				$("#myCurrentTime2" + $(videos[p]).attr("id")).show();
			}
			else {
				...
				// 播放
				$(videos[p]).attr("myCurrentTime2", 0);
				$(videos[p]).attr("myMaxTime2", 0);
				$("#myCurrentTime2" + $(videos[p]).attr("id")).hide();
			}
			videos[p].addEventListener('canplay', function (e) {
				...
				if ($(videos[this.num]).attr("myCurrentTime2") - videos[this.num].currentTime > 1) {
					videos[this.num].currentTime = $(videos[this.num]).attr("myCurrentTime2");
				}
			});
			...
			videos[p].addEventListener('pause', function (e) {
				e.preventDefault();
				var seeTime = secToTime($(videos[this.num]).attr("myCurrentTime2"));
				$("#myCurrentTime" + $(videos[this.num]).attr("id")).text("上次观看" + seeTime);
				$("#myCurrentTime2" + $(videos[this.num]).attr("id")).text("上次观看" + seeTime);
	...
	$.ajax({
		...
		success: function (data) {
			if (data.retcode == "0") {
				...
				$("#viewNum2" + id).text(data.allDemandVolume);
				$("#viewName2" + id).attr("style", "color:#848484");
			} else {
...
function successVideo(id) {
	let useridid = GetQueryString('userid');
	if (!useridid || useridid == null) {
		useridid == '';
	}
	console.log('我是视频id:' + id);
	console.log('我是分享者d:' + useridid);
	$.ajax({
		url: appVideo.updateVideoFee + "?code=" + wxcode,
		type: 'POST',
		data: JSON.stringify({
			'share_userId': useridid,
			'videoId': id
		}),
		beforeSend: function (xhr) {
			//$.showLoading();
...
//tab切换逻辑 已完成   待学习
$('#tabIndex span').click(function () {
	//tab标题切换
	$(this).attr('class', 'active');
	$(this).siblings().attr('class', '');
	let itemId = $(this).attr('tabid');
		appVideo.learnstatus = itemId;
	...
	$.ajax({
		...
		success: function (data) {
			...
			if (data.retcode == "0") {
				let list = data.list;
				if (list.length > 0) {
					window.location.href = './lookList.html?voideId=' + id;
				}
...
//去分享
function toShare(id) {
	window.location.href = './shareVideo.html?voideId=' + id;
}
//播放
function play(id) {
	// console.log(id,'temp')
	// $('.page1').hide();
	// $('.page2').show();
	// appVideo.fanhui = true;
	// bofang(id)
	window.location.href = './shareVideo.html?voideId=' + id;
}
//获取视频列表2 及 观看次数
function bofang(id) {
	var list = appVideo.splist
	console.log(list);
	var htmls = "";
	var videoTemp = $('#videoBoxTempage2').html();
	for (var i = 0; i < list.length; i++) {
		if (id == list[i].id) {
			var temp = {};
			temp.id = list[i].id;
...
$(window).scroll(function () {
	...
	console.log(windowH + scrollH + 70,'windowH + scrollH + 70')
	console.log(windowH + scrollH + 70>= documentH,'70')
	if (windowH + scrollH + 70>= documentH) {
		console.log(appVideo.loading,'appVideo.loading')
		if (appVideo.loading) {
			appVideo.pageNum++;
			queryVideoList1()
		}
