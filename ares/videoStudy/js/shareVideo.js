var appVideo = {
    ...
    VolumeRealUrl :base.context + "qywx/learnVideo/saveDemandVolumeReal.xa", // 点击观看
    ...
    lookMum:base.context + "qywx/learnVideo/getVideoLookNum.xa", //观看次数
    saveFeeAndUpdataData:base.context + 'learnVideo/saveFeeAndUpdataData.xa', // 视频学习完成记录及记录积分
    base:'/wxqy',
    initFlag:false,
    saveCacheFlag:true,
    keyWord:"",
...
$(document).ready(function(){
    ...
    wx.ready(function(){
        ...
        wx.showMenuItems({
            menuList:['menuItem:share:appMessage','menuItem:share:wechat']
        });
    });
...
function queryVideoList1(){
    voideid = GetQueryString('voideId');
    $.ajax({
        ...
        success : function(data) {
            if(data.retcode=="0"){
                $('.videoWrap').html("");
                $('.notips').hide();
                appVideo.content = data.data.title;
                let list = [];
                list.push(data.data)
                console.log(list);
                if(list.length>0){
...
function getLookMun1(){
    voideid = GetQueryString('voideId');
    $.ajax({
        url : appVideo.lookMum+"?code="+wxcode,
        type : 'POST',
        data : JSON.stringify({
            'videoId': voideid
        }),
        ...
        success : function(data) {
            if(data.retcode=="0"){
                console.log(data);
                $(".viewNum").text(data.data);
                lookNum = data.data;
                $('.lookNums').html(lookNum);
    ...
    for (var p = videos.length - 1; p >= 0; p--) {
        (function(){
            ...
            videos[p].addEventListener('play',function(e){
                ...
                VolumeReal($(videos[this.num]).attr("id"))
                setInit(this.num);
                restarInterval($(videos[this.num]).attr("id")) // 播放开始10秒记录
            ...
            videos[p].addEventListener('timeupdate',function(e){
                ...
                if(appVideo.initFlag){
                    ...
                    if (time > 0 && current == time) {
                        saveFeeAndUpdataData($(videos[this.num]).attr("id"));
                    }
                }
            });
        })();
...
function VolumeReal (id) {
    //纪录用户观看视频
    useridid = GetQueryString('userid');
    if(!useridid || useridid == null){
        useridid == '';
    }
    // alert(useridid);
    $.ajax({
        url : appVideo.VolumeRealUrl,
        type : 'POST',
        data : JSON.stringify({
...
// 观看成功
function saveFeeAndUpdataData(id){
    useridid = GetQueryString('userid');
    if(!useridid || useridid == null){
        useridid == '';
    }
	...
	console.log('我是分享者d:'+useridid);
    $.ajax({
        url : appVideo.saveFeeAndUpdataData,
        type : 'POST',
        data : JSON.stringify({
            'share_userId':useridid,
            'videoId': id
...
function share() {
    ...
    let imgPath = "https://weixin.xacbank.com.cn/qywx/videoStudy/image/background_top.png";
    let linkPath = '';
    if(window.location.href.indexOf('uat_wxqy') > 0) {
        console.log('我是准生产');
        wx.ready(function () {
            wx.onMenuShareTimeline({
                title: '学习视频', // 分享标题
                link: linkPath,
                imgUrl: window.location.href.split("/videoStudy/")[0] + "/videoStudy/shareVideo.html?type=" + appVideo.id + "&content=" + encodeURIComponent(content) + "&videoType=" + appVideo.secondVideoType + "&voideId=" + voideid + "&userid=" + JSON.parse($.cookie("user")).userid, // 分享图标
                success: function () {
                },
                cancel: function () {
                }
            ...
            wx.onMenuShareAppMessage({
                ...
                link: window.location.href.split("/videoStudy/")[0] + "/videoStudy/shareVideo.html?type=" + appVideo.id + "&content=" + encodeURIComponent(content) + "&videoType=" + appVideo.secondVideoType + "&voideId=" + voideid + "&userid=" + JSON.parse($.cookie("user")).userid,
                imgUrl: imgPath, // 分享图标
...
function lookRecord (id) {
    window.location.href = './count.html?videoId='+id;
}
